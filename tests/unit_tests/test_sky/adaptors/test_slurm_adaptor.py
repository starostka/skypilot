...
class TestJobSteps:
    ...
    def test_list_job_steps_runs_as_allocation_owner(self):
        ...
        assert transport.call_args.args[0] == _with_marker(
...
def _with_marker(cmd) -> str:
    ...
    if isinstance(cmd, list):
        # Mirrors _run_slurm_cmd: a list is shlex.join'd before the marker
        # echo is prepended, so the expectation has to make the same turn.
        cmd = shlex.join(cmd)
