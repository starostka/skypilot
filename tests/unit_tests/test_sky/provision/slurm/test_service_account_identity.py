...
def test_submit_validation_uses_bounded_submit_runner():
    with mock.patch(
        ...
        # The argv reaches `command_as_user` rather than `run`: carrying the
        # login-banner marker means _run_slurm_cmd must hand the runner a
        # string, so it makes the allocation-owner wrap itself rather than
        # letting run() do it. Same wrap, one layer up.
        assert (runner.return_value.command_as_user.call_args.args[0] == [
            'id', '-un'
        ])
...
def test_submit_validation_accepts_login_banner(stdout):
    ...
    # Prefixed with the login-banner marker echo: _run_slurm_cmd emits one
    # before every command so a site whose profile.d writes to stdout cannot
    # feed its banner to the parsers. The wrapped argv is unchanged.
    assert transport.call_args.args[0] == (
        f'echo {slurm.SlurmClient._OUTPUT_MARKER}\n'  # pylint: disable=protected-access
