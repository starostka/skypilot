...
class SlurmClient:
    ...
    #
    # `_run_slurm_cmds` solves the same problem for its own framed transport
    # with _BATCH_OUTPUT_HEADER, and independently -- it has to, because the
    # noise would corrupt its base64 frames. This covers the SINGLE-command
    # path, which that header never reaches.
    ...
    def _run_slurm_cmd(self,
        ...
        # A list has to become a string to carry the marker echo -- but the
        # list/str distinction is LOAD-BEARING in the runner: a list means
        # "run this argv as the allocation owner" and is sudo-wrapped by
        # SlurmLoginNodeCommandRunner.command_as_user, a string means "run
        # this shell code as the SSH user" and is not. Collapsing a list with
        # a bare shlex.join here would silently drop submit-as-user for every
        # argv command. So make the same turn the runner would have made,
        # then hand it the string.
        if isinstance(cmd, list):
            command_as_user = getattr(self._runner, 'command_as_user', None)
            # LocalProcessCommandRunner (in-cluster autodown) has no such
            # concept: there is no SSH user to differ from.
            cmd = (command_as_user(cmd)
                   if command_as_user is not None else shlex.join(cmd))
        cmd = f'echo {self._OUTPUT_MARKER}\n{cmd}'
